...
for dat_file in "$datfiles_folder"/*.dat; do
    if [ -f "$dat_file" ]; then
        ...
        openfoam2306 blockMesh # generate the mesh
        openfoam2306 simpleFoam | tee logOpt.txt #run in parallel
        ...
        python3 csvwrite.py "$datfiles_folder" "$coeff_data" "$dat_filename" #Pass the necessary parameters here
